...
'''
cap = cv2.VideoCapture('smashvid.mp4')
timeLength = 558.0 #actually 558.0
fps = 30
frame = 20
while(cap.isOpened() && frame < fps*timeLength):
	frameCalc = (frame/(timeLength*fps))
	cap.set(2,frameCalc)
	frame = frame + 150
	...
	while arranged:
		pt = arranged.pop(0)
		print("Comparing " + str(pt[1]) + " to " + str(lastPt))
		if pt[1] - lastPt[1] < 10:
			print "DISQUALIFIED"
		else:
			print "SCHWIFTY"
